...
import {
  ...
  CreditCard, Sparkles, Monitor, Moon, Smartphone, Sliders, Hash,
} from "lucide-react";
...
function SettingCard({ icon: Icon, label, desc, children }: { icon: any; label: string; desc: string; children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3 p-4 rounded-2xl bg-white/[0.02] border border-white/[0.06] hover:border-white/[0.1] transition-all">
...
NEXT_PUBLIC_INSTAGRAM_URL="${branding.instagram}"`;
  ...
  async function testWebhook() {
    ...
    try {
      ...
      setTestResult(data.result || data.error || "Sent");
  ...
  return (
    <div className="mobile-section max-w-4xl mx-auto">
      ...
      {/* Tabs */}
      <div className="flex items-center gap-1 mb-6 overflow-x-auto premium-scrollbar">
        {TABS.map((t) => {
          const Icon = t.icon;
          ...
          return (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-medium font-space whitespace-nowrap transition-all ${
                active ? "accent-bg-subtle accent-text" : "text-slate-400 hover:text-white bg-white/[0.03] hover:bg-white/[0.06]"
              }`}>
              ...
              {[
                ...
                <button key={c.value} onClick={() => { setAccent(c.value); applyAccent(c.value); }}
                  className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all ${
                    accent === c.value ? "ring-2 ring-[var(--accent)] border-transparent" : "border-white/10"
                  }`}>
      ...
      {activeTab === "branding" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="space-y-5">
            <div className="premium-glass-strong rounded-2xl md:rounded-3xl p-5 md:p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500/10 to-teal-500/10 flex items-center justify-center">
            ...
            <div className="premium-glass-strong rounded-2xl md:rounded-3xl p-5 md:p-6">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-white font-space">Environment Snippet</span>
                </div>
        ...
        <div className="space-y-5">
          <div className="premium-glass-strong rounded-2xl md:rounded-3xl p-5 md:p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-sm font-semibold text-white font-space">Stripe</h2>
                <p className="text-[10px] text-slate-500 font-space">Payments, invoices &amp; subscriptions</p>
              </div>
            </div>
            ...
            <a href="https://dashboard.stripe.com/apikeys" target="_blank"
              className="inline-flex items-center gap-1.5 text-xs text-blue-400 hover:text-blue-300 font-space">
              Manage in Stripe Dashboard <ExternalLink size={11} />
            </a>
          </div>
        </div>
      )}
            ...
            <div className="space-y-3 max-w-md">
              <div>
                ...
                <select value={testType} onChange={(e) => setTestType(e.target.value)}
                  className="w-full px-3.5 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-sm text-white focus:outline-none focus:border-[var(--accent)]/40 font-space">
      ...
      {activeTab === "system" && (
        <div className="space-y-5">
          ...
          <div className="premium-glass-strong rounded-2xl md:rounded-3xl p-5 md:p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-semibold text-white font-space">System Statistics</h2>
                  <p className="text-[10px] text-slate-500 font-space">Live platform overview</p>
                </div>
              </div>
              <button onClick={refreshStats} disabled={refreshing} className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/5 transition-all">
          ...
          <div className="premium-glass-strong rounded-2xl md:rounded-3xl p-5 md:p-6">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-red-500/10 to-orange-500/10 flex items-center justify-center">
                <Zap size={16} className="text-red-400" />
              </div>
              <div>
                <h2 className="text-sm font-semibold text-white font-space">Admin Tools</h2>
                <p className="text-[10px] text-slate-500 font-space">Management utilities</p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Link href="/dashboard/data"
                ...
                <div>
                  <div className="text-sm font-medium text-white font-space">Clear Test Data</div>
                  <div className="text-[10px] text-slate-500 font-space">Remove all non-admin data</div>
                </div>
              </Link>
              <a href="https://vercel.com" target="_blank"
                ...
                <div>
                  <div className="text-sm font-medium text-white font-space">Vercel Dashboard</div>
                  <div className="text-[10px] text-slate-500 font-space">Deployments &amp; analytics</div>
                </div>
              </a>
            </div>
          </div>
