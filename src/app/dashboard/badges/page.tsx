...
import { Search, Shield, Check, Plus, X, Sparkles, RefreshCw, Users, Trash2 } from "lucide-react";

const BADGE_OPTIONS = [
  { id: "VERIFIED", label: "Verified", color: "accent", icon: "✓" },
  { id: "PRO", label: "Pro", color: "purple", icon: "⭐" },
  { id: "EARLY_SUPPORTER", label: "Early Supporter", color: "amber", icon: "🔥" },
  { id: "ADMIN", label: "Admin", color: "red", icon: "⚡" },
];
  ...
  async function toggleBadge(userId: string, badge: string) {
    ...
    if (res.ok) {
      ...
      setUserBadges((prev) => {
        ...
        return {
          ...prev,
          [userId]: data.added ? [...current, badge] : current.filter((b) => b !== badge),
        };
      });
  ...
  async function clearAllBadges() {
    ...
    for (const user of users) {
      ...
      for (const badge of badges) {
        if (badge === "ADMIN") continue; // Preserve admin badge
      ...
      <div className="flex items-center gap-3 mb-6 md:mb-8">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs md:text-sm text-slate-500 font-space">
            {users.length} users · {totalBadges} badges assigned
          </p>
        </div>
        {totalBadges > 0 && (
          <button onClick={() => setConfirmClear(true)}
            ...
            title="Clear all non-admin badges">
            ...
            return (
              <div key={u.id} className="group premium-card-hover premium-glass-strong rounded-2xl p-4 md:p-5 border border-white/[0.06]">
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className={`text-[8px] px-2 py-0.5 rounded-full font-semibold font-space ${
                    u.role === "ADMIN" ? "bg-red-500/10 text-red-400" : "bg-white/[0.04] text-slate-400"
                  }`}>{u.role}</span>
                </div>
                  ...
                  {BADGE_OPTIONS.map((badge) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={isToggling}
                        className={`inline-flex items-center gap-1 text-[10px] px-2.5 py-1 rounded-xl font-medium font-space border transition-all ${
                          has ? colors.active : colors.inactive
                        } disabled:opacity-60`}
                      >
                        {isToggling ? <RefreshCw size={10} className="animate-spin" /> : <>{isU}</>}
                        {badge.label}
                      </button>
                    );
                  })}
