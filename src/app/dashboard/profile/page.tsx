...
export default function AdminProfile() {
  ...
  async function saveProfile(e: React.FormEvent) {
    ...
    if (res.ok) {
      setSaved(true);
      setInitialForm(form); // mark clean
      showToast("Profile updated successfully", "success");
      setTimeout(() => setSaved(false), 3000);
      fetchProfile(); // will also refresh initial inside fetch
    } else {
  ...
  // Simple crop states for image cropping
  ...
  const handleDragOver = (e: React.DragEvent, field: "image" | "banner") => {
    e.preventDefault();
    e.stopPropagation();
    setDragOver(field);
  };
  ...
  // Load + cache the crop image (supports both avatar square + banner wide)
  ...
  // Aspect-aware source rect (square for avatar, wide 3:1 for banner)
  function computeSourceRect(
    ...
    // Desired src rect dimensions
    let srcH = Math.min(naturalH, naturalW / aspect) / Math.max(0.35, scale);
    let srcW = srcH * aspect;

    if (srcW > naturalW) {
      srcW = naturalW / Math.max(0.35, scale);
      srcH = srcW / aspect;
    }

    // Center with pan offset (tuned travel)
  ...
  // High quality output crop (different size/aspect per type)
  const applyCrop = async () => {
    ...
    const doCrop = async (imageEl: HTMLImageElement) => {
      ...
      const { sx, sy, sw, sh } = computeSourceRect(
        imageEl.width, imageEl.height, cropScale, cropOffsetX, cropOffsetY, aspect
      );
      ...
      canvas.toBlob(async (blob) => {
        if (blob) {
          const ext = cropFile.name.replace(/\.[^.]+$/, ".jpg");
          const croppedFile = new File([blob], ext, { type: "image/jpeg" });
          await uploadFile(cropType, croppedFile);
          closeCrop();
        }
      }, "image/jpeg", 0.93);
    };
    ...
    if (cropImageRef.current) {
      await doCrop(cropImageRef.current);
    } else {
  ...
  // Fast live redraw (adapts preview canvas size + guide for banner vs avatar)
  const redrawCropCanvas = () => {
    ...
    function drawFrom(imageEl: HTMLImageElement) {
      ...
      if (isBanner) {
        // Wide rounded rectangle guide for banner
        const m = 5;
        const rw = cw - m * 2;
        const rh = ch - m * 2;
        ctx.beginPath();
        ctx.roundRect(m, m, rw, rh, 10);
        ctx.stroke();
      } else {
        // Circle for avatar
        const cx = cw / 2;
        ...
        ctx.setLineDash([]);
        ctx.beginPath();
        ctx.moveTo(cx - 12, cx); ctx.lineTo(cx + 12, cx);
        ctx.moveTo(cx, cx - 12); ctx.lineTo(cx, cx + 12);
        ctx.stroke();
      }
      ctx.restore();
    }
  ...
  // Redraw on param change + when crop type changes
  useEffect(() => {
  ...
  // Load image when a new file is chosen for cropping
  useEffect(() => {
  ...
  // Basic mouse drag support for crop preview
  ...
  // Track original for dirty state
  ...
  const onCropMouseMove = (e: React.MouseEvent) => {
    ...
    // Smooth higher-sensitivity pan
    const dx = (e.clientX - dragStart.x) * 0.65;
    const dy = (e.clientY - dragStart.y) * 0.65;
    setCropOffsetX(prev => Math.max(-160, Math.min(160, prev + dx)));
    setCropOffsetY(prev => Math.max(-160, Math.min(160, prev + dy)));
    setDragStart({ x: e.clientX, y: e.clientY });
  };
  ...
  // getPlatformLabel + getSocialIcon now centralized in @/lib/utils for consistency (icons everywhere)
  ...
  return (
    <div className="max-w-2xl mx-auto">
      {/* Live preview hero like social media (updates as you edit) */}
      <div className="relative mb-8 -mx-4 md:mx-0">
        ...
        >
          ...
          {/* Quick remove for banner */}
          {form.banner && (
            <button
              type="button"
              onClick={() => clearField("banner")}
              className="absolute top-2 right-2 z-10 bg-black/60 hover:bg-black/80 text-white/80 hover:text-white p-1 rounded-full transition-all"
              title="Remove banner"
            >
              <X size={13} />
            </button>
          ...
          <div className="absolute bottom-2 right-2 text-[10px] bg-black/50 px-1.5 py-0.5 rounded text-white/70 hidden group-hover:block">Drop image here</div>
          <input
            ref={bannerInputRef}
            type="file"
            accept="image/*"
          ...
          >
            <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl ring-4 ring-[#050505] bg-blue-500/20 border border-blue-500/30 overflow-hidden shadow-xl">
              {form.image ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-3xl font-bold text-blue-400">
                  {(form.displayName || form.name || "U").charAt(0).toUpperCase()}
                </div>
              )}
            </div>
            <button
            ...
            {form.image && (
              <button
                ...
                className="absolute -top-1 -right-1 z-10 bg-black/70 hover:bg-red-500/80 text-white p-0.5 rounded-full transition-all opacity-0 group-hover:opacity-100"
                title="Remove photo"
              >
                <X size={11} />
              </button>
            )}
            <div className="absolute -bottom-1 right-1 text-[8px] bg-black/60 px-1 py-0.5 rounded text-white/70 hidden group-hover:block">Drop here</div>
            <input
              ref={imageInputRef}
              type="file"
              accept="image/*"
      ...
      <form onSubmit={saveProfile} className="space-y-6">
        <div className="glass p-6 rounded-xl border border-white/10">
          ...
          <p className="text-xs text-slate-500 mb-3 font-space">Use the live hero preview above to drag, drop or click (avatar will be cropped square). Or paste URLs below.</p>
          <div 
            className={`space-y-4 rounded-lg transition-all ${dragOver ? "ring-1 ring-blue-400/40" : ""}`}
            onDragOver={(e) => handleDragOver(e, dragOver || "image")}
            onDragLeave={handleDragLeave}
            onDrop={(e) => handleDrop(e, "image")}
          >
            <div className="flex items-center gap-4 flex-wrap">
              {/* Live small avatar preview */}
              ...
              <div className="space-y-2 flex-1 min-w-[210px]">
                {/* Avatar URL row + actions */}
                <div className="flex gap-2 flex-wrap items-center">
                  <input
                    placeholder="Profile image URL"
                    value={form.image}
                    onChange={(e) => setForm({ ...form, image: e.target.value })}
                    className="flex-1 min-w-[160px] px-4 py-2.5 rounded-lg bg-slate-800/50 border border-white/10 text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500/50 font-space text-sm"
                  />
                  <input
                    ref={imageInputRef}
                    type="file"
                  ...
                  <button
                    ...
                    onClick={() => imageInputRef.current?.click()}
                    disabled={uploading === "image"}
                    className="px-4 py-2.5 rounded-lg border border-white/10 text-slate-300 hover:text-white hover:border-white/20 text-sm transition-all disabled:opacity-50 font-space shrink-0"
                  >
                    {uploading === "image" ? "..." : "Upload"}
                  </button>
                  {form.image && (
                    <button type="button" onClick={() => clearField("image")} className="px-3 py-2.5 text-xs text-red-400/80 hover:text-red-400 transition-colors font-space">Remove</button>
                  )}
                </div>

                {/* Banner URL row + actions */}
                <div className="flex gap-2 flex-wrap items-center">
                  <input
                  ...
                  {form.banner && (
                    <button type="button" onClick={() => clearField("banner")} className="px-3 py-2.5 text-xs text-red-400/80 hover:text-red-400 transition-colors font-space">Remove</button>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="glass p-6 rounded-xl border border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-slate-500 mb-1 font-space">Username</label>
              <input
                value={form.username}
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1 font-space">Display Name</label>
              <input
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1 font-space">Full Name</label>
              <input
                value={form.name}
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1 font-space">Email</label>
              <input
                value={user?.email ?? ""}
                disabled
                className="w-full px-4 py-2.5 rounded-lg bg-slate-800/30 border border-white/5 text-slate-500 font-space text-sm cursor-not-allowed"
              />
            </div>
          </div>
        </div>
        ...
        {/* Social Links - add business socials/websites as clickable badges */}
        <div className="glass p-6 rounded-xl border border-white/10">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-300 uppercase tracking-wider mb-4 font-space">
            <Globe size={15} className="text-blue-400" /> Social Links &amp; Website
          </h2>
          <p className="text-xs text-slate-500 mb-3 font-space">Add links — they appear as clickable badges on your profile (and preview).</p>

          {/* Current links as removable pills */}
          {form.socialLinks && form.socialLinks.length > 0 && (
              ...
              {form.socialLinks.map((link, idx) => (
                <div key={idx} className="flex items-center gap-1.5 bg-white/5 border border-white/10 rounded-full pl-3 pr-1 py-0.5 text-xs font-space">
                  ...
                  <a 
                    ...
                    className="text-blue-400 hover:underline max-w-[140px] truncate"
                  ...
                  <button 
                    ...
                    className="ml-1 text-slate-400 hover:text-red-400 px-1"
          ...
          <div className="flex flex-wrap gap-2 items-end">
            <div>
              ...
              <select 
                ...
                className="px-3 py-2 rounded-lg bg-slate-800/50 border border-white/10 text-sm text-white font-space"
              >
            ...
            {newPlatform === "other" && (
              <input 
                ...
                className="px-3 py-2 rounded-lg bg-slate-800/50 border border-white/10 text-sm text-white placeholder:text-slate-500 font-space min-w-[120px]"
              />
            )}
            <div className="flex-1 min-w-[180px]">
              ...
              <input 
                ...
                className="w-full px-3 py-2 rounded-lg bg-slate-800/50 border border-white/10 text-sm text-white placeholder:text-slate-500 font-space"
              />
            </div>
            <button 
              type="button" 
              onClick={addSocialLink}
              className="px-4 py-2 rounded-lg border border-white/10 hover:bg-white/5 text-sm font-space transition-all text-slate-200"
            >
              + Add
            </button>
          </div>
          <div className="text-[10px] text-slate-500 mt-2 font-space">Links will show as badges below your name and are clickable in the full preview / public profile.</div>
        </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                onClick={() => {
                  ...
                  showToast("Linktree URL copied to clipboard", "success");
                }}
      ...
      {user?.badges && user.badges.length > 0 && (
        <div className="glass p-6 rounded-xl border border-white/10 mt-6">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-300 uppercase tracking-wider mb-4 font-space">
            <ImageIcon size={15} className="text-blue-400" /> Your Badges
          </h2>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div className="glass rounded-2xl border border-white/10 p-6 max-w-lg w-full">
              ...
              return (
                <>
                  ...
                  <p className="text-xs text-slate-400 mb-3 font-space">
                    Drag the image to reposition • Use slider to zoom. The {isBanner ? "outline" : "circle"} shows your final crop.
                  </p>
                  ...
                  <div 
                    ...
                    style={{ 
                      ...
                      width: isBanner ? "min(100%, 420px)" : 300 
                    }}
                  >
                    <canvas 
                      ref={cropCanvasRef} 
                    ...
                    />
                    {/* Bottom hint bar */}
                    <div className="absolute bottom-0 left-0 right-0 bg-black/70 text-[10px] text-center py-1.5 text-slate-400 font-space tracking-wide">
                      Drag to pan • Adjust zoom below
                    </div>
                  </div>
                </>
              );
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="flex justify-between mb-1.5">
                    <label className="text-xs text-slate-400 font-space">Pan X</label>
                    <span className="text-[10px] text-slate-500 tabular-nums font-mono">{Math.round(cropOffsetX)}</span>
                  </div>
                  <input 
                    type="range" 
                    min="-160" 
                ...
                <div>
                  <div className="flex justify-between mb-1.5">
                    <label className="text-xs text-slate-400 font-space">Pan Y</label>
                    <span className="text-[10px] text-slate-500 tabular-nums font-mono">{Math.round(cropOffsetY)}</span>
                  </div>
                  <input 
                    type="range" 
                    min="-160" 
            ...
            <div className="flex gap-3">
              <button 
                onClick={() => { setCropScale(1.1); setCropOffsetX(0); setCropOffsetY(0); }}
                className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-white/10 text-sm text-slate-300 hover:bg-white/5 font-space"
              >
                <RotateCcw size={14} /> Reset
              </button>
              <button 
                onClick={closeCrop}
                className="flex-1 py-2.5 rounded-xl border border-white/10 text-sm hover:bg-white/5 font-space"
              >
                Cancel
              </button>
              <button 
                onClick={applyCrop}
                className="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium font-space inline-flex items-center justify-center gap-2"
              >
                <Check size={15} /> Crop &amp; Use
              </button>
            </div>
          </div>
        </div>
